...
gRPC ports used by Dgraph Alpha and Dgraph Zero, see [Ports Usage](/docs/deploy/security/ports-usage).
...
replaced by the `--tls` [superflag](/docs/deploy/cli-command-reference) 
...
Some examples of connecting via a [Client](/docs/dql/clients) when TLS is in use can be found below:
