...
If [ACL](/docs/access-control-lists) is enabled then you can log-in to the default namespace (`0`) with the following method:
...
If [multi-tenancy](/docs/enterprise-features/multitenancy) is enabled, by default the login method on client will login into the namespace `0`.
...
setup using the instructions provided [here](/docs/deploy/security/tls-configuration).
