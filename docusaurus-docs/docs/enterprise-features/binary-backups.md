...
the backup. Unlike [exports](/docs/deploy/admin/dgraph-administration#exporting-database),
...
[whitelisted for admin operations](/docs/deploy/admin/dgraph-administration#whitelisting-admin-operations).
...
An example of this use-case would be to migrate existing schemas from Dgraph v1.0 to Dgraph .
You need to update the schema file from an export so all predicates of type `uid` are changed to `[uid]`.
Then use the updated schema when loading data into Dgraph .
...
Dgraph's [Encryption at Rest](/docs/enterprise-features/encryption-at-rest) uses a symmetric-key
...
You use the `--vault` [superflag](/docs/deploy/cli-command-reference) to specify the [Hashicorp Vault](https://www.vaultproject.io/) server address (`addr`), role id (`role-id-file`), secret id (`secret-id-file`) and the field that contains the encryption key (`enc-field`) that was used to encrypt the backup.
