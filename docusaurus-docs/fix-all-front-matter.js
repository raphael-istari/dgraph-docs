...
function fixFrontMatter(filePath) {
  ...
  // Fix titles and descriptions with special characters
  let fixedFrontMatter = frontMatter
    .replace(/^title: (.+)$/gm, (match, title) => {
    ...
    .replace(/^description: (.+)$/gm, (match, description) => {
      // If description contains special characters, quote it
      if (description.includes('@') || description.includes(':') || description.includes('"') || description.includes("'") || description.includes('&')) {
...
console.log('Comprehensive front matter fixes complete!');
