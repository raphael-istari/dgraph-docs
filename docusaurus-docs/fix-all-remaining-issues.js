...
function fixAllRemainingIssues(filePath) {
    ...
    // Fix all unclosed HTML tags by converting them to HTML entities
    const htmlTags = [
        'NodeUID', 'node2', 'objectUID', 'object', 'Type', 'RelationshipName',
...
function processDirectory(directory) {
    ...
    files.forEach(file => {
        ...
        if (stat.isDirectory()) {
            ...
            fixAllRemainingIssues(filePath);
        }
    });
}

processDirectory(docsDir);
console.log('All remaining issues fixes complete!');
