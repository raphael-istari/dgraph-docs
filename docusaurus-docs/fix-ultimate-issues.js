...
function fixUltimateIssues(filePath) {
    ...
    const htmlTags = [
        ...
        'value', 'ip-address', 'port', 'addr', 'enc-field', 'enc-format', 'path'
    ];
    ...
    // Fix the specific issue with <*ip-address*> and <*port*> syntax
    ...
    if (newContent !== fileContent) {
        ...
        console.log(`Fixed ultimate issues in: ${filePath}`);
    }
}
...
function processDirectory(directory) {
    ...
    files.forEach(file => {
        ...
        if (stat.isDirectory()) {
            ...
            fixUltimateIssues(filePath);
        }
    });
}

processDirectory(docsDir);
console.log('Ultimate issues fixes complete!');
